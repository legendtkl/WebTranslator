(() => {
  ...
  /**
   * Extract translatable elements (paragraphs and headings) from the DOM.
   */
  function extractTranslatableElements(root) {
    console.log('WebTranslator: [DEBUG] extractTranslatableElements called with NEW logic');
    const elements = [];
    ...
    console.log('WebTranslator: [DEBUG] Final element count before return:', elements.length);
    return elements;
  }
  ...
  function applyTranslations(elements, translatedTexts, displayMode = 'bilingual') {
    ...
    elements.forEach((element, i) => {
      if (!translatedTexts[i]) {
        console.log('WebTranslator: No translation for element', i, element);
        return;
      }
  ...
  function restoreOriginalText() {
    ...
    translatedElements.forEach(element => {
      ...
      element.classList.remove('web-translator-translated', 'web-translator-loading');
  ...
  async function translatePage(sourceLang = 'auto', targetLang = 'zh', displayMode = 'bilingual') {
    ...
    // Split into batches for progressive translation
    ...
    for (let i = 0; i < texts.length; i += batchSize) {
      ...
      elementBatches.push(validElements.slice(i, i + batchSize));
    }
    ...
    try {
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        console.log(`WebTranslator: Processing batch ${batchIndex + 1}/${batches.length}`, batch.length, 'elements');
